...
public sealed class LocalizationService : ObservableObject
{
    ...
    private static readonly IReadOnlyDictionary<string, (string Turkish, string English)> Strings =
        new Dictionary<string, (string Turkish, string English)>
        {
            ...
            ["PausedWindowBackground"] = ("Duraklatıldı (pencere arka planda)", "Paused (window in background)"),
            ["RunningWindowMinimized"] = ("Çalışıyor (pencere simge durumunda)", "Running (window minimized)"),
            ["ErrorWindowClosed"] = ("Hata - pencere kapatıldı", "Error - window closed"),
            ["FastDialogueStressTest"] = ("Hızlı diyalog stres testi", "Fast dialogue stress test"),
            ["OcrRunning"] = ("OCR çalışıyor", "OCR running"),
            ["Ready"] = ("Hazır", "Ready"),
            ["Translating"] = ("Çeviriyor…", "Translating…"),
            ["OverlayNotStarted"] = ("Başlatılmadı", "Not started"),
